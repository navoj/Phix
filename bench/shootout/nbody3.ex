...
procedure advance(atom dt)
    atom tempMass, dx, dy, dz, distance, mag
    sequence tempCoord, tempV, delta, delta2

    for i = 1 to NBODIES do
	tempMass = bodies[i][MASS]
	tempCoord = bodies[i][X..Z]
	tempV = bodies[i][VX..VZ]
	for j = i + 1 to NBODIES do
	    delta = tempCoord - bodies[j][X..Z]
	    delta2 = delta * delta
	    distance = sqrt(delta2[1] + delta2[2] + delta2[3])
	    mag = dt / (distance*distance*distance)

	    tempV -= delta * bodies[j][MASS] * mag
	    bodies[j][VX..VZ] += delta * tempMass * mag
	end for
	bodies[i][X..Z] = tempCoord
	bodies[i][VX..VZ] = tempV
     end for

    for i = 1 to NBODIES do
	bodies[i][X..Z] += dt * bodies[i][VX..VZ]
    end for
