...
integer syntaxClass
--trace(1)
--  abcl = length(abc) -- removed 4/3/2013, see below
    while chidx2<=lt do
        ch2 = text[chidx2]
        if ch2>128 then
            ctype = TokenChar
...
--  bl = 7
    while chidx<=lt do
        ch = text[chidx]
--if ch<=0 then exit end if --DEV?? [+1??]
        if ch>128 then
            ctype = TokenChar
        else
            ctype = charMap[ch+1]
...
--DOUBLE DEV this only does first closing block comment...
--          abc = blockComment[2]  -- removed 4/3/2013
            chidx2 -= 1
            scanForUrls(text)
            syntaxClass = Comments
...
--                  if find(word,WordLists[newSyntax][i-Other-7]) then
                    if find(word,WordLists[newSyntax][i-BookMarks-7]) then
                        syntaxClass = i
                        if equal(LowerCase(word),"include")
