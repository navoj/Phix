...
--  puts(1, "\n\t\tUsing Debug Version of machine.e\n")
    puts(1, "\n\t\tUsing Debug Version of machine.e (untested!)\n")
atom t
    t = time()
    while time()<t+3 do
    end while
            ...
            if block[3]>=1 then
                ...
                or (start>=block_upper and
                    ...
                    return BAD
                       ...
                       upper<=block_start)
                   or (upper>block_upper and
                       upper<block_upper+BORDER_SPACE) then

                    return BAD

                elsif start<block_start-BORDER_SPACE
                  and upper>block_upper+BORDER_SPACE then
...
-- makes warning beeps first so you can see what's happening on the screen
    ...
    --/* Not required for Phix
    if machine_func(5, -1) then end if -- graphics_mode
    --*/
    puts(1, "\n\n" & msg & "\n\n")
    if getc(0) then end if
    ? 1/0 -- force traceback
...
global
procedure poke(atom a, object v)
...
global
procedure poke4(atom a, object v)
...
global
procedure mem_copy(machine_addr target, machine_addr source, natural len)
-- safe mem_copy
...
procedure original_mem_set(atom target, atom val, integer len)
    mem_set(target, val, len)
end procedure

global
procedure mem_set(machine_addr target, atom val, natural len)
-- safe mem_set
    if safe_address(target, len) then
        original_mem_set(target, val, len)
    else
...
atom start
    ...
    if len<=30 then
        -- show whole block
        for i=floor(start) to floor(start+len-1) do
            show_byte(i)
        end for
    else
        -- first part of block
        for i=floor(start) to floor(start+14) do
            show_byte(i)
        end for
        -- last part of block
        puts(1, "\n ...\n")
        printf(1, "#%x: ", start+len-15)
        for i=floor(start+len-15) to floor(start+len-1) do
            show_byte(i)
        end for
    end if
    bad = 0
    -- check post-block
    for i=floor(start+len) to floor(start+len+BORDER_SPACE-1) do
        p = original_peek(i)
...
global
procedure c_proc(integer i, sequence s)
    original_c_proc(i, s)
    if check_calls then
        check_all_blocks()
    end if
...
global
function c_func(integer i, sequence s)
object r
...
global
procedure unregister_block(machine_addr block_addr)
...
--/**/  a = n+BORDER_SPACE*2                                    --      -- Phix
--/**/  #ilASM{ lea edi,[a]     -- lea edi,[result]             --      -- Phix
--/**/          lea esi,[a]     -- lea esi,[size]               --      -- Phix
--/**/          call %opAlloc}  -- [edi] = allocate([esi])      --      -- Phix
--/**/                                                          --/*    -- Phix
        a = machine_func(M_ALLOC, n+BORDER_SPACE*2)             --         RDS --*/
...
--/**/      a -= BORDER_SPACE                                       --   -- Phix
--/**/      free(a)                                                 --/* -- Phix
            machine_proc(M_FREE, a-BORDER_SPACE)                    --      RDS --*/
            -- remove it from list
            safe_address_list = safe_address_list[1..i-1] &
                                safe_address_list[i+1..length(safe_address_list)]
            return
...
global
function bytes_to_int(sequence s)
...
global
function int_to_bits(atom x, integer nbits)
